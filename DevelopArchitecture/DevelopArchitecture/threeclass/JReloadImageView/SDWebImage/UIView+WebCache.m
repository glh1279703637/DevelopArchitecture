...
- (NSNumber*)m_currentIsPlaceholder {
    return objc_getAssociatedObject(self, @selector(m_currentIsPlaceholder));
}

- (void)setM_currentIsPlaceholder:(NSNumber*)m_currentIsPlaceholder{
    objc_setAssociatedObject(self, @selector(m_currentIsPlaceholder), m_currentIsPlaceholder, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSNumber*)m_imageviewContentMode {
    return objc_getAssociatedObject(self, @selector(m_imageviewContentMode));
}

- (void)setM_imageviewContentMode:(NSNumber*)m_imageviewContentMode{
    objc_setAssociatedObject(self, @selector(m_imageviewContentMode), m_imageviewContentMode, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
    ...
    if (!(options & SDWebImageDelayPlaceholder)) {
        dispatch_main_async_safe(^{
            self.m_currentIsPlaceholder = @(YES);
...
- (void)sd_setImage:(UIImage *)image imageData:(NSData *)imageData basedOnClassOrViaCustomSetImageBlock:(SDSetImageBlock)setImageBlock cacheType:(SDImageCacheType)cacheType imageURL:(NSURL *)imageURL {
    ...
    if (setImageBlock) {
        ...
        __weak UIImageView *weakImageView = (UIImageView *)view;
        finalSetImageBlock = ^(UIImage *setImage, NSData *setImageData, SDImageCacheType setCacheType, NSURL *setImageURL) {
            __strong UIImageView *imageView = weakImageView;
            if(imageView.m_imageviewContentMode != nil){
                imageView.contentMode = imageView.m_imageviewContentMode.integerValue;
            }
    ...
    if([self isKindOfClass:[UIImageView class]]){
        if(self.m_currentIsPlaceholder.integerValue == 1){
            self.m_imageviewContentMode = @(self.contentMode);
            self.contentMode = UIViewContentModeScaleToFill;
        }else{
            if(self.m_imageviewContentMode != nil){
                self.contentMode = self.m_imageviewContentMode.integerValue;
                self.m_imageviewContentMode = nil;
            }
        }
    }
    self.m_currentIsPlaceholder = nil;
}
#endif
