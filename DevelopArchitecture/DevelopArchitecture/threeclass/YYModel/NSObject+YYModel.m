...
- (instancetype)initWithClass:(Class)cls {
    ...
    if ([cls respondsToSelector:@selector(funj_modelCustomPropertyMapperPrefix)]) {
        NSString *subPrefix = [(id <YYModel>)cls funj_modelCustomPropertyMapperPrefix];
        NSMutableDictionary *customMappers = [[NSMutableDictionary alloc]init];
        [allPropertyMetas enumerateKeysAndObjectsUsingBlock:^(NSString *name, _YYModelPropertyMeta *propertyMeta, BOOL *stop) {
            if([name hasPrefix: subPrefix]){
                [customMappers setObject:[name substringFromIndex:subPrefix.length] forKey:name];
            }
        }];
        customMapper = customMappers;
    }
    if(customMapper.count > 0){
