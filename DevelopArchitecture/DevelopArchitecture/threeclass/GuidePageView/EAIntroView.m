...
#pragma clang diagnostic pop
    ...
    if([page isEqual:[_pages lastObject]]){
        UITapGestureRecognizer *tap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(introDidFinish111)];
...
- (void)buildFooterView {
    ...
    [self.skipButton setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.currentPageIndex == (pageViews.count)) {
        if (self.flag == 0) {
            ...
            if ([(id)self.delegate respondsToSelector:@selector(backView)]) {
...
- (void)makePanelVisibleAtIndex:(NSInteger)panelIndex{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        for (int idx = 0; idx < strongSelf->pageViews.count; idx++) {
            if (idx == panelIndex) {
                [strongSelf->pageViews[idx] setAlpha:1];
            }
            else {
                if(!self.hideOffscreenPages) {
                    [strongSelf->pageViews[idx] setAlpha:0];
                }
            }
        }
    }];
