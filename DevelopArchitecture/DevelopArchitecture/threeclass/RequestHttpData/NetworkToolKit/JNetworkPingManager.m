//
//  JNetworkPingManager.m
//  testApp
//
//  Created by Jeffrey on 2020/8/21.
...
static JNetworkPingManager *networkPingManager = nil;

@interface JNetworkPingManager ()<SimplePingDelegate>

@property(nonatomic,strong)SimplePing *m_pinger;
@property(nonatomic,strong)NSTimer *m_sendTimer;
@property(nonatomic,assign)NSInteger m_countPingSendToReceive,m_countTotalPingSend;

@property(nonatomic,assign)NSInteger m_countPingError,m_countPingErrorShow;

@end

@implementation JNetworkPingManager
+(instancetype)share{
    if(!networkPingManager){
        networkPingManager =[[JNetworkPingManager alloc]init];
    }
    return networkPingManager;
}

-(void)funj_startPing:(CGFloat)repeatTime{
    NSString * hostName = self.m_hostName ?: @"baidu.com";

    self.m_pinger = [[SimplePing alloc]initWithHostName:hostName];
//      if (forceIPv4 && !forceIPv6) {
    self.m_pinger.addressStyle = SimplePingAddressStyleICMPv4;
//      } else if (forceIPv6 && !forceIPv4) {
//          self.m_pinger.addressStyle = SimplePingAddressStyleICMPv6;
//      }
    self.m_pinger.delegate = self;
    [self.m_pinger start];
    
    __weak typeof(self) weakSelf = self;
    self.m_sendTimer =[NSTimer scheduledTimerWithTimeInterval:repeatTime repeats:YES block:^(NSTimer * _Nonnull timer) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        [strongSelf.m_pinger sendPingWithData:nil];
    }];
}
...
-(void)funj_StopPing{
    [self.m_pinger stop];
    self.m_pinger = nil;
    if(self.m_sendTimer.isValid)[self.m_sendTimer invalidate];
    self.m_sendTimer = nil;
    networkPingManager = nil;
}

#pragma mark delegate
-(void)simplePing:(SimplePing *)pinger didFailWithError:(NSError *)error{
    self.m_countPingError ++ ;
...
-(void)simplePing:(SimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber{
    if(self.m_countTotalPingSend % 20 == 0){
    ...
    self.m_countTotalPingSend ++;
    CLog(@"#%u sent %zd", sequenceNumber,self.m_countPingSendToReceive);
}
-(void)simplePing:(SimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error{
    self.m_countPingSendToReceive --;
    CLog(@"#%u send failed: %@", sequenceNumber, error);

}
-(void)simplePing:(SimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber{
    self.m_countPingSendToReceive --;
    CLog(@"#%u received, size=%zu", sequenceNumber, packet.length);
}
-(void)simplePing:(SimplePing *)pinger didReceiveUnexpectedPacket:(NSData *)packet{
    CLog(@"unexpected packet, size=%zu", packet.length);
}
@end
