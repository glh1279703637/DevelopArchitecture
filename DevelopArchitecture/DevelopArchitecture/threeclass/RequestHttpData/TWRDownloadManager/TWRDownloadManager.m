...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite {
    ...
    TWRDownloadObject *download = [self.downloads objectForKey:fileIdentifier];
    download.downloadState=IsLoadingDown_Tag;
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    NSString *fileIdentifier = downloadTask.originalRequest.URL.absoluteString;
    TWRDownloadObject *download = [self.downloads objectForKey:fileIdentifier];
    download.downloadState=AlreadlyFinishDown_Tag;
    ...
    [[NSFileManager defaultManager] moveItemAtURL:location
                                            ...
                                            error:&error];
    if (error) {
         download.downloadState=ErrorDownLoad_tag;
...
- (BOOL)fileDownloadCompletedForUrl:(NSString *)fileIdentifier {
    BOOL retValue = YES;
    TWRDownloadObject *download = [self.downloads objectForKey:fileIdentifier];
    if(download.downloadState==DownloadPrepare_tag){
        [download.downloadTask cancel];
        [self.downloads removeObjectForKey:fileIdentifier];
        download=nil;
    }
...
- (NSString *)localPathForFile:(NSString *)fileIdentifier inDirectory:(NSString *)directoryName {
    ...
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    NSString *cachesDirectory = [paths objectAtIndex:0];
    if(directoryName)cachesDirectory = [cachesDirectory stringByAppendingPathComponent:directoryName];
...
- (BOOL)fileExistsWithName:(NSString *)fileName
    ...
    if(![[NSFileManager defaultManager] fileExistsAtPath:[cachesDirectory stringByAppendingPathComponent:directoryName?directoryName:@""]] && directoryName){
          [[NSFileManager defaultManager] createDirectoryAtPath:[cachesDirectory stringByAppendingPathComponent:directoryName] withIntermediateDirectories:YES attributes:nil error:nil];
    }
    
    if ([[NSFileManager defaultManager] fileExistsAtPath:[[cachesDirectory stringByAppendingPathComponent:directoryName?directoryName:@""] stringByAppendingPathComponent:fileName]]) {
        exists = YES;
    }
        ...
        if ([downloadTasks count] == 0) {
            if (self.backgroundTransferCompletionHandler != nil) {
                ...
                [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                    ...
                    [JAppUtility funj_postLocalNotifation:@"下载提示" s:nil b:@"所有文件资源已下载完成" i:nil];
                }];
