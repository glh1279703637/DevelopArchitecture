...
-(void)setDDPageControl{
    _page=[[DDPageControl alloc]init];
    _page.center=CGPointMake(self.frame.size.width/2-_page.frame.size.width/2, self.frame.size.height-10);
    //        _page.defersCurrentPageDisplay=YES;
    _page.indicatorSpace=4;
    _page.indicatorDiameter=7;
    _page.enabled=NO;
...
{    
    ...
    _page.frame=CGRectMake(self.frame.size.width/2-_page.frame.size.width/2, _page.frame.origin.y, _page.frame.size.width, _page.frame.size.height);
    _page.numberOfPages = _numberOfItems;
...
- (void)startAnimation
{
    if (!_timer)
    {
        self.timer = [NSTimer timerWithTimeInterval:1/24.0
                                             target:self
...
- (void)step
{
    ...
    double delta = 0;//currentTime - _lastTime;
...
//        self.scrollOffset = [self clampedOffset:_scrollOffset - delta * _autoscroll];
       ...
       if(_numberOfItems) index = (index+1 + _numberOfItems) %_numberOfItems;
        [self scrollToItemAtIndex:index  duration:3];
    }
...
- (void)didScroll
{
    ...
    _page.currentPage = self.currentItemIndex;
