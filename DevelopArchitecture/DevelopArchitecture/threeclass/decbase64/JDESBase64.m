...
+(NSString*)encryptionKey:(NSString*)key{
    if(!key)key=@"DevelopArchitecture";
    else key=[key stringByAppendingString:@"DevelopArchitecture"];
    NSData *data = [key dataUsingEncoding:NSUTF8StringEncoding];
    NSString *dataStr=[JAppUtility funj_md5:[data description]];
    [dataStr stringByAppendingString:@"O(#_@+KDOW(@>DI(!||{#(@12"];
    return [JAppUtility funj_md5:dataStr];
}
...
+ (NSString *)funj_hmacsha1:(NSString *)data k:(NSString *)key {
    ...
    NSString *hash = [self funj_base64EncodedStringFrom:HMAC];
