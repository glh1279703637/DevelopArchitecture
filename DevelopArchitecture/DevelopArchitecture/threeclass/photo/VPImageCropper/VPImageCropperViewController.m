...
- (void)initView {
    ...
    oriWidth = self.cropFrame.size.width*3/2;
    ...
    if(self.isCircleCropFrame)self.ratioView.layer.cornerRadius = self.ratioView.frame.size.width/2;
    self.ratioView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [self.view addSubview:self.ratioView];
    
    [self overlayClipping];
    
}
-(void)setBackBgImageView:(UIImageView *)backBgImageView{
    if(_backBgImageView){
        [_backBgImageView removeFromSuperview];
    }
    _backBgImageView = backBgImageView;
    if(!backBgImageView)return;
    self.view.backgroundColor = [UIColor whiteColor];
    
    [self.view insertSubview:backBgImageView belowSubview:self.ratioView];
}
...
-(UIImage *)getSubImage{
    ...
    UIImageView *imageView =[self.view viewWithTag:300];
    if(!imageView)imageView =[[UIImageView alloc]initWithFrame:CGRectMake(10, 10, 200, 200)];
